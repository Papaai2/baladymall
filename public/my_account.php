...
$header_path_from_public = __DIR__ . '/../src/includes/header.php'; // Starts session
...
if (!isset($_SESSION['user_id'])) {
    header("Location: " . rtrim(SITE_URL, '/') . "/login.php?auth=required");
    exit;
}

$user_id = $_SESSION['user_id'];
$current_username = isset($_SESSION['username']) ? $_SESSION['username'] : 'N/A';
$current_first_name = isset($_SESSION['first_name']) ? $_SESSION['first_name'] : '';
$current_last_name = isset($_SESSION['last_name']) ? $_SESSION['last_name'] : '';
$current_email = isset($_SESSION['email']) ? $_SESSION['email'] : 'N/A';
$current_phone_number = isset($_SESSION['phone_number']) ? $_SESSION['phone_number'] : '';
$current_role = isset($_SESSION['role']) ? $_SESSION['role'] : 'customer';

$full_name = trim($current_first_name . ' ' . $current_last_name);
if (empty($full_name)) {
    $full_name = htmlspecialchars($current_username);
} else {
...
$orders_list = []; // For storing fetched orders

// --- Current Shipping Address Variables ---
...
$current_shipping_country = 'Egypt'; // Default

if ((isset($_GET['view']) && $_GET['view'] == 'addresses') || (isset($_POST['update_shipping_address']))) {
    try {
        if (!(isset($_POST['update_shipping_address']) && empty($address_errors) && !empty($address_success_message))) {
        ...
        $address_errors['form'] = "Could not load your current address. Please try again.";
    }
}

// --- Fetch Orders if view is 'orders' ---
    ...
    if (isset($_POST['update_profile'])) {
        ...
        if (empty($profile_errors)) {
            ...
            if ($new_phone_number !== $current_phone_number && !empty($new_phone_number) ) { 
                ...
                $stmt_check_phone->bindParam(':phone_number', $new_phone_number);
                $stmt_check_phone->bindParam(':user_id', $user_id, PDO::PARAM_INT);
        ...
        if (empty($profile_errors)) {
            try {
                $update_sql = "UPDATE users SET first_name = :first_name, last_name = :last_name, email = :email, phone_number = :phone_number WHERE user_id = :user_id";
                ...
                $full_name = htmlspecialchars(trim($current_first_name . ' ' . $current_last_name));
                if (empty(trim($full_name))) { $full_name = htmlspecialchars($current_username); }
        ...
        if (empty($password_errors)) {
            try { 
                ...
                if ($user_data && password_verify($current_password_input, $user_data['password'])) {
                    ...
                    $stmt_update_pass = $db->prepare("UPDATE users SET password = :password WHERE user_id = :user_id");
        ...
        $_GET['view'] = 'change_password';

    } elseif (isset($_POST['update_shipping_address'])) {
        // ... (shipping address update logic remains the same) ...
        ...
        $new_shipping_country = trim(filter_input(INPUT_POST, 'shipping_country', FILTER_UNSAFE_RAW));
        ...
        if (empty($address_errors)) {
            try {
                $update_address_sql = "UPDATE users SET shipping_address_line1 = :addr1, shipping_address_line2 = :addr2, shipping_city = :city, shipping_governorate = :gov, shipping_postal_code = :zip, shipping_country = :country WHERE user_id = :user_id";
                ...
                if ($stmt_update_address->rowCount() >= 0) { 
                    ...
                    $address_errors['form'] = "Failed to update shipping address. Please try again.";
                }
            } catch (PDOException $e) {
                error_log("Shipping Address Update Error: " . $e->getMessage());
...
<div class="account-page-container">
    <aside class="account-sidebar">
        ...
        <nav class="sidebar-nav">
            <ul>
                <li class="nav-item <?php echo (!isset($_GET['view']) || $_GET['view'] == 'dashboard') ? 'active' : ''; ?>"><a href="<?php echo rtrim(SITE_URL, '/'); ?>/my_account.php?view=dashboard" class="nav-link">Dashboard</a></li>
                <li class="nav-item <?php echo (isset($_GET['view']) && $_GET['view'] == 'orders') ? 'active' : ''; ?>"><a href="<?php echo rtrim(SITE_URL, '/'); ?>/my_account.php?view=orders" class="nav-link">Order History</a></li>
                <li class="nav-item <?php echo (isset($_GET['view']) && $_GET['view'] == 'profile') ? 'active' : ''; ?>"><a href="<?php echo rtrim(SITE_URL, '/'); ?>/my_account.php?view=profile" class="nav-link">My Details</a></li>
                <li class="nav-item <?php echo (isset($_GET['view']) && $_GET['view'] == 'addresses') ? 'active' : ''; ?>"><a href="<?php echo rtrim(SITE_URL, '/'); ?>/my_account.php?view=addresses" class="nav-link">Shipping Addresses</a></li>
                <li class="nav-item <?php echo (isset($_GET['view']) && $_GET['view'] == 'change_password') ? 'active' : ''; ?>"><a href="<?php echo rtrim(SITE_URL, '/'); ?>/my_account.php?view=change_password" class="nav-link">Change Password</a></li>
                <?php if ($current_role === 'brand_admin'): ?>
                    <li class="nav-item separator"><hr></li>
                    <li class="nav-item"><a href="<?php echo rtrim(SITE_URL, '/'); ?>/../brand_admin/index.php" class="nav-link btn btn-info btn-sm">Brand Dashboard</a></li>
                <?php elseif ($current_role === 'super_admin'): ?>
                    <li class="nav-item separator"><hr></li>
                    <li class="nav-item"><a href="<?php echo rtrim(SITE_URL, '/'); ?>/../admin/index.php" class="nav-link btn btn-warning btn-sm">Super Admin Panel</a></li>
                <?php endif; ?>
                <li class="nav-item separator"><hr></li>
    ...
    <section class="account-content">
        <div class="content-header"><h1><?php echo $full_name; ?></h1>
            <?php if (isset($_GET['login']) && $_GET['login'] === 'success' && (!isset($_GET['view']) || $_GET['view'] == 'dashboard')) {
        ...
        <div class="content-body">
            ...
            // Display relevant messages based on the current view
            if ($view === 'profile') {
                if (!empty($profile_success_message)) echo "<div class='form-message success-message'>" . htmlspecialchars($profile_success_message) . "</div>";
                if (!empty($profile_errors['form'])) echo "<div class='form-message error-message'>" . htmlspecialchars($profile_errors['form']) . "</div>";
            } elseif ($view === 'change_password') {
                if (!empty($password_success_message)) echo "<div class='form-message success-message'>" . htmlspecialchars($password_success_message) . "</div>";
                if (!empty($password_errors['form'])) echo "<div class='form-message error-message'>" . htmlspecialchars($password_errors['form']) . "</div>";
            } elseif ($view === 'addresses') {
                if (!empty($address_success_message)) echo "<div class='form-message success-message'>" . htmlspecialchars($address_success_message) . "</div>";
                if (!empty($address_errors['form'])) echo "<div class='form-message error-message'>" . htmlspecialchars($address_errors['form']) . "</div>";
            }
            ...
            switch ($view) {
                case 'orders':
                    ...
                    if (!empty($orders_list)) {
                        echo "<table class='orders-table'>"; // Add class for styling
                        echo "<thead><tr><th>Order ID</th><th>Date</th><th>Total</th><th>Status</th><th>Action</th></tr></thead>";
                        echo "<tbody>";
                        foreach ($orders_list as $order) {
                            echo "<tr>";
                            echo "<td>#" . htmlspecialchars($order['order_id']) . "</td>";
                            echo "<td>" . htmlspecialchars(date("F j, Y, g:i a", strtotime($order['order_date']))) . "</td>";
                            echo "<td>" . CURRENCY_SYMBOL . htmlspecialchars(number_format($order['total_amount'], 2)) . "</td>";
                            echo "<td>" . htmlspecialchars(ucfirst(str_replace('_', ' ', $order['order_status']))) . "</td>";
                            echo "<td><a href='" . rtrim(SITE_URL, '/') . "/order_details.php?order_id=" . htmlspecialchars($order['order_id']) . "' class='btn btn-sm btn-secondary'>View Details</a></td>"; // Placeholder link
                            echo "</tr>";
                        }
                        echo "</tbody>";
                        echo "</table>";
                ...
                case 'profile':
                    ...
                    echo "<input type='text' id='first_name' name='first_name' value='" . htmlspecialchars($current_first_name) . "' required>";
                    if (isset($profile_errors['first_name'])) echo "<span class='error-text'>" . htmlspecialchars($profile_errors['first_name']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='last_name'>Last Name <span class='required'>*</span></label>";
                    echo "<input type='text' id='last_name' name='last_name' value='" . htmlspecialchars($current_last_name) . "' required>";
                    if (isset($profile_errors['last_name'])) echo "<span class='error-text'>" . htmlspecialchars($profile_errors['last_name']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='email'>Email Address <span class='required'>*</span></label>";
                    echo "<input type='email' id='email' name='email' value='" . htmlspecialchars($current_email) . "' required>";
                    if (isset($profile_errors['email'])) echo "<span class='error-text'>" . htmlspecialchars($profile_errors['email']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='phone_number'>Phone Number <span class='required'>*</span></label>";
                    echo "<input type='tel' id='phone_number' name='phone_number' value='" . htmlspecialchars($current_phone_number) . "' required placeholder='+201XXXXXXXXX'>";
                    if (isset($profile_errors['phone_number'])) echo "<span class='error-text'>" . htmlspecialchars($profile_errors['phone_number']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='username_display'>Username (cannot be changed)</label>";
                    echo "<input type='text' id='username_display' name='username_display' value='" . htmlspecialchars($current_username) . "' readonly disabled>";
                ...
                case 'addresses':
                    ...
                    // Shipping address form (restored)
                    ...
                    echo "<input type='text' id='shipping_address_line1' name='shipping_address_line1' value='" . htmlspecialchars($current_shipping_address_line1) . "' required>";
                    if (isset($address_errors['shipping_address_line1'])) echo "<span class='error-text'>" . htmlspecialchars($address_errors['shipping_address_line1']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='shipping_address_line2'>Address Line 2 (Optional)</label>";
                    echo "<input type='text' id='shipping_address_line2' name='shipping_address_line2' value='" . htmlspecialchars($current_shipping_address_line2) . "'>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='shipping_city'>City <span class='required'>*</span></label>";
                    echo "<input type='text' id='shipping_city' name='shipping_city' value='" . htmlspecialchars($current_shipping_city) . "' required>";
                    if (isset($address_errors['shipping_city'])) echo "<span class='error-text'>" . htmlspecialchars($address_errors['shipping_city']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='shipping_governorate'>Governorate <span class='required'>*</span></label>";
                    echo "<input type='text' id='shipping_governorate' name='shipping_governorate' value='" . htmlspecialchars($current_shipping_governorate) . "' required>";
                    if (isset($address_errors['shipping_governorate'])) echo "<span class='error-text'>" . htmlspecialchars($address_errors['shipping_governorate']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='shipping_postal_code'>Postal Code (Optional)</label>";
                    echo "<input type='text' id='shipping_postal_code' name='shipping_postal_code' value='" . htmlspecialchars($current_shipping_postal_code) . "'>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='shipping_country'>Country <span class='required'>*</span></label>";
                    echo "<input type='text' id='shipping_country' name='shipping_country' value='" . htmlspecialchars($current_shipping_country) . "' required>";
                    if (isset($address_errors['shipping_country'])) echo "<span class='error-text'>" . htmlspecialchars($address_errors['shipping_country']) . "</span>";
                    echo "</div>";
                    echo "</fieldset>";
                    echo "<div class='form-group'>";
                ...
                case 'change_password':
                    ...
                    if (isset($password_errors['current_password'])) echo "<span class='error-text'>" . htmlspecialchars($password_errors['current_password']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    echo "<label for='new_password'>New Password <span class='required'>*</span></label>";
                    echo "<input type='password' id='new_password' name='new_password' required>";
                    if (isset($password_errors['new_password'])) echo "<span class='error-text'>" . htmlspecialchars($password_errors['new_password']) . "</span>";
                    echo "</div>";
                    echo "<div class='form-group'>";
                    ...
                    if (isset($password_errors['confirm_password'])) echo "<span class='error-text'>" . htmlspecialchars($password_errors['confirm_password']) . "</span>";
                    echo "</div>";
                    echo "</fieldset>";
                    echo "<div class='form-group'>";
