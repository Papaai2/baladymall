<?php
// brand_admin/products.php - Brand Admin: Manage Products
...
if (file_exists($main_config_path)) {
    ...
    die("CRITICAL BRAND ADMIN PRODUCTS ERROR: Main config.php not found.");
}
...
// Products are filtered by the brand_id from the session
...
if (!empty($where_clauses)) {
    $sql_products .= " AND " . implode(" AND ", $where_clauses);
...
try {
    ...
    error_log("Brand Admin Products - Error fetching product count for brand {$current_brand_id}: " . $e->getMessage());
    echo "<div class='brand-admin-message error'>Error fetching product count.</div>";
}
...
$products = [];
if ($total_records > 0 || empty($where_clauses)) { // Attempt to fetch if records exist or no filters applied
    try {
...
// --- Function to delete a single product and its related data (brand-specific) ---
function deleteSingleProduct($db, $product_id_to_delete, $brand_id_owner) {
    // IMPORTANT: Verify product belongs to this brand_id_owner before deleting
    $stmt_check_owner = $db->prepare("SELECT brand_id, main_image_url FROM products WHERE product_id = :pid");
    $stmt_check_owner->bindParam(':pid', $product_id_to_delete, PDO::PARAM_INT);
    $stmt_check_owner->execute();
    $product_info = $stmt_check_owner->fetch(PDO::FETCH_ASSOC);

    if (!$product_info || $product_info['brand_id'] != $brand_id_owner) {
        // Product does not exist or does not belong to this brand admin
    ...
    try {
        ...
        // This is crucial to allow deletion if items are past 'pending'/'processing'.
        ...
        $stmt_delete_product = $db->prepare("DELETE FROM products WHERE product_id = :product_id AND brand_id = :brand_id"); // Double-check brand_id
        $stmt_delete_product->bindParam(':product_id', $product_id_to_delete, PDO::PARAM_INT);
        $stmt_delete_product->bindParam(':brand_id', $brand_id_owner, PDO::PARAM_INT);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_product'])) {
    ...
    if ($product_id_to_delete && $csrf_token_form && hash_equals($_SESSION['csrf_token'], $csrf_token_form)) {
        $result = deleteSingleProduct($db, $product_id_to_delete, $current_brand_id);
        if ($result['success']) {
            $_SESSION['brand_admin_message'] = "<div class='brand-admin-message success'>{$result['message']}</div>";
        } else {
            $_SESSION['brand_admin_message'] = "<div class='brand-admin-message error'>{$result['message']}</div>";
        }
        // Refresh page
        header("Location: products.php?page={$page}" . ($filter_status ? "&filter_status={$filter_status}" : ""));
        exit;
    } else {
        $_SESSION['brand_admin_message'] = "<div class='brand-admin-message error'>Invalid request for deletion or CSRF token mismatch.</div>";
        header("Location: products.php?page={$page}" . ($filter_status ? "&filter_status={$filter_status}" : ""));
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['batch_delete_products'])) {
    ...
    if (!empty($product_ids_to_delete) && $csrf_token_form && hash_equals($_SESSION['csrf_token'], $csrf_token_form)) {
        ...
        foreach ($product_ids_to_delete as $pid) {
            $result = deleteSingleProduct($db, $pid, $current_brand_id);
            if ($result['success']) {
                $deleted_count++;
            } else {
        ...
        $_SESSION['brand_admin_message'] = "<div class='brand-admin-message warning'>No products selected for batch deletion.</div>";
        header("Location: products.php?page={$page}" . ($filter_status ? "&filter_status={$filter_status}" : ""));
        exit;
    } else {
         $_SESSION['brand_admin_message'] = "<div class='brand-admin-message error'>Invalid request for batch deletion or CSRF token mismatch.</div>";
        header("Location: products.php?page={$page}" . ($filter_status ? "&filter_status={$filter_status}" : ""));
        exit;
    }
}
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
    ...
    <div style="display: flex; gap: 10px;">
        <a href="add_product.php" class="btn-submit" style="display: inline-block; padding: 8px 15px; text-decoration:none;">+ Add New Product</a>
        <a href="batch_upload_products.php" class="btn-submit" style="display: inline-block; padding: 8px 15px; text-decoration:none; background-color: #6c757d; border-color: #6c757d;">Batch Upload</a>
    </div>
</div>
...
if (isset($_SESSION['brand_admin_message'])) {
    ...
    unset($_SESSION['brand_admin_message']); // Clear the message after displaying
}
?>
